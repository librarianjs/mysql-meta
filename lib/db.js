...
function createDBConnection( options ){
  ...
  var tableName = options.tableName || 'librarian_uploads'
  ...
  function create( data ){
    ...
    var insertQueryString =
      'INSERT INTO ' + database + '.' + table + '(' + keys.join( ', ' ) + ')\n' +
      'VALUES (' + values.map( function(){ return '?' }).join( ', ' ) + ');\n' +
      'SELECT LAST_INSERT_ID() as lastInsert;'

    return new Promise( function( resolve, reject ){
      connection.query( insertQueryString, values, function( err, records ){
        if( err ){
          reject( err )
        } else {
          var lastInsertId = records[0].lastInsert
          findOne( lastInsertId ).then( function( record ){
            resolve( record )
          }, reject )
        }
      })
